...
void tcp_socket_init()
{
	...
	printstr_log("Initialized TCP socket\n");
}
...
void tcp_connect_timeout(void *_args)
{
	...
	switch(socket->state) {
		case SYN_SENT:
			socket->state = CLOSED;
			printstr_log("tcp: SYN_SENT -> CLOSED\n");
			task_wakeup(socket);
			break;
		case CLOSED:
		case LISTEN:
	...
	switch(socket->state) {
		case CLOSED:
			...
			*args = socket_id;
			...
			printstr_log("tcp: CLOSED -> SYN_SENT\n");
...
int tcp_socket_listen(int socket_id)
{
	...
	switch(socket->state) {
		case CLOSED:
			socket->state = LISTEN;
			printstr_log("tcp: CLOSED -> LISTEN\n");
			return 0;
		case SYN_SENT:
		case CLOSE_WAIT:
		case LISTEN:
...
int tcp_socket_close(int socket_id)
{
	...
	switch(socket->state) {
		case ESTABLISHED:
			...
			printstr_log("tcp: ESTABLISHED -> FIN_WAIT_1\n");
			task_sleep(socket);
...
void handle_ack(struct tcp_socket *socket, struct tcp_hdr *tcphdr){
	...
	//detect dup ack. check retransmit necessity
	printstr_log("DETECTED DUPPED ACK: ");
	printnum_log(ntoh32(tcphdr->ack_num));
	printstr_log("\n");
	for(tx_data = (struct tcp_tx_data *)list_head(&socket->tx_data_list);
			tx_data != NULL; tx_data = (struct tcp_tx_data *)list_next(&tx_data->link)){
		if(tx_data->send_seq_num == ntoh32(tcphdr->ack_num)) {
			tx_data->dup_ack_num++;
			printstr_log("dup_count: ");
			printnum_log(tx_data->dup_ack_num);
			printstr_log("\n");
			if(tx_data->dup_ack_num >= RETRANSMIT_DUP_ACK_NUM) {
				printstr_log("retransmit: ");
				printnum_log(tx_data->txpkt->pkt_len);
				printstr_log("\n");
	...
	switch(socket->state) {
		...
		case LISTEN:
			if((ntoh16(tcphdr->flags) & TCP_FLAGS_SYN)) {
				...
				printstr_log("tcp: LISTEN -> SYN_RCVD\n");
			}
			break;
		case SYN_RCVD:
			pkt->buf += tcphdr_len;
			if((ntoh16(tcphdr->flags) & TCP_FLAGS_ACK)) {
				...
				if(data_len > 0) {
					//printstr_log("ack in 3 hand shake contains data!!\n");
					// copy pkt data
				...
				printstr_log("tcp: SYN_RCVD -> ESTABLISHED\n");
				task_wakeup(socket);
			}
			break;
		case SYN_SENT:
			if((ntoh16(tcphdr->flags) & TCP_FLAGS_SYN) && (ntoh16(tcphdr->flags) & TCP_FLAGS_ACK)) {
				...
				socket->state = ESTABLISHED;
				printstr_log("tcp: SYN_SENT -> ESTABLISHED\n");
				task_wakeup(socket);
			}
			break;
		case ESTABLISHED:
			...
			if((ntoh16(tcphdr->flags) & TCP_FLAGS_FIN) && (ntoh16(tcphdr->flags) & TCP_FLAGS_ACK)) {
				...
				printstr_log("tcp: ESTABLISHED -> CLOSING\n");
			} else if (data_len > 0) {
				tcp_send(socket_id, socket->dip, socket->dport, NULL, 0, TCP_FLAGS_ACK, false);
			}
			break;
		case FIN_WAIT_1:
			...
			if((ntoh16(tcphdr->flags) & TCP_FLAGS_FIN) && (ntoh16(tcphdr->flags) & TCP_FLAGS_ACK)) {
				...
				tcp_send(socket_id, socket->dip, socket->dport, NULL, 0, TCP_FLAGS_ACK, false);
				socket->state = CLOSED;
				printstr_log("tcp: FIN_WAIT_1 -> CLOSED\n");
				task_wakeup(socket);
			}
			break;
		case CLOSING:
			if((ntoh16(tcphdr->flags) & TCP_FLAGS_ACK)) {
				socket->state = CLOSED;
				printstr_log("tcp: CLOSING -> CLOSED\n");
				task_wakeup(socket);
			}
			break;
		default:
