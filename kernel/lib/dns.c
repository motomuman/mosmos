...
int parse_domain_name(uint8_t *rbuf, uint8_t *wbuf, int wlen)
{
	...
	while(*rbuf != 0) {
		if(*rbuf == 0xc0) {
			printstr_app("ERROR can not parse offset\n");
			return -1;
		}
		int nextlen = *rbuf;
		rbuf+=1;
		wlen -= nextlen;
		if(wlen < 0) {
			printstr_app("ERROR not enough space to write parsed domain name\n");
			return -1;
		}
...
uint32_t resolve_addr(int sock, char *name)
{
	...
	udp_socket_send(sock, resolver_ip, 53, txbuf, sizeof(struct dns_hdr) + qsection_len);

	int ret = udp_socket_recv(sock, rxbuf, 500);
	if(ret == -1) {
		printstr_app("Failed to resolve dns\n");
		mem_free(txbuf_head);
		mem_free(rxbuf_head);
		return 0;
	}
	...
	if(ntoh16(ansdns_hdr->qdcount) != 1){
		printstr_app("Can not handle dns response with non one qd count (");
		printnum_app(ntoh16(ansdns_hdr->qdcount));
		printstr_app(") \n");
		mem_free(txbuf_head);
		mem_free(rxbuf_head);
		return 0;
	...
	if(*rxbuf != 0xc0){
		printstr_app("Can not handle dns response not using offset\n");
		mem_free(txbuf_head);
		mem_free(rxbuf_head);
		return 0;
	}
	...
	if(ntoh16(*length) != 4){
		printstr_app("Can not handle dns response with non 4 length\n");
		mem_free(txbuf_head);
		mem_free(rxbuf_head);
		return 0;
	}
...
int resolve_host(int sock, uint32_t ip, char *buf, int buflen)
{
	...
	if(ntoh16(ansdns_hdr->ancount) == 0){
		printstr_app("Can not handle dns response with zero ancount\n");
		mem_free(txbuf_head);
		mem_free(rxbuf_head);
		return -1;
	}
